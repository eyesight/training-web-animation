document.addEventListener("DOMContentLoaded", function(){
	...
	const tl_loader = new TimelineMax({delay:0.5, onComplete:logoVisible});
	const tl_run = new TimelineMax({delay:0.5, repeat:2});
	const tl_eye = new TimelineMax({delay:1.5, repeat:3, repeatDelay: 1});

	//start-animation
	tl.set(backcircle, {y:35, x:40})
	  .set(bodybetween, {scaleX: 0.5, y:35, x:45})
	  .set(frontcircle, {y:35, x:10})
	  .set(head, {y:20})
	  .set(eyes, {y:20})
	  .set(ears, {y:20})
	  .set(tail, {y:110, x:30, rotation:0})
	  .set(backlegs, {rotation:-100, y:55, x:50});
	...
	tl_loader.set(loadingbar, {opacity: 1})
			 .fromTo(loader, 3, {opacity: 1, width:"0%"}, {width:"70%"}, "+=0.5")
			 .to(loader, 2, {width:"100%"})
