document.addEventListener("DOMContentLoaded", function(){
	...
	const aliencow = document.querySelector('.buddycow');
	const aliensnail = document.querySelector('.buddysnail');
	const alienunicorn = document.querySelector('.buddyunicorn');
	...
	for (let i = 0, length = radiobanimal.length; i < length; i++){
		...
		radiobanimal[i].addEventListener('click', function (event) {
			...
			if(content){
				...
				alienunicorn.classList.remove('visible');
				aliencow.classList.remove('visible');
				aliensnail.classList.remove('visible');
				switch(content) {
				    case 'cow':
				        aliencow.classList.add('visible');
				        break;
				    case 'snail':
				        aliensnail.classList.add('visible');
				        break;
				    case 'unicorn':
				        alienunicorn.classList.add('visible');
				        break;
				    default:
				        alienunicorn.classList.remove('visible');
						aliencow.classList.remove('visible');
